...
gulp.task('webpack', ['clean-webpack', 'test', 'bundle'], () => webpack(require('./webpack.config.js'))
    .pipe(gulp.dest('www/js')));

gulp.task('revision', ['webpack'], () => gulp.src(['./www/js/*.js'])
    .pipe(rev())
    .pipe(through.obj(addToManifest))
    .pipe(gulp.dest('www/js')));
...
gulp.task('mustache-dev', ['static', 'revision', 'pack-css'], () => gulp.src('templates/*.mustache')
    .pipe(mustache({}, {}, {
...
gulp.task('mustache-min', ['static', 'revision', 'pack-css-min'], () => gulp.src('templates/*.mustache')
    .pipe(mustache({}, {}, {
...
gulp.task('build', ['pack-css', 'revision', 'mustache-dev'], () => gulp.src('www/**')
    .pipe(connect.reload()));

gulp.task('build-min', ['pack-css-min', 'revision', 'mustache-min'], () => gulp.src('www/**')
    .pipe(connect.reload()));
