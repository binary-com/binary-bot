...
const expectOptions = options => {
  ...
  if (!symbol) {
    const error = createError(
      'OptionError',
      translate('Underlying market is not selected'),
    );
    globalObserver.emit('Error', error);
    throw error;
  }
  ...
  if (!contractTypes[0]) {
    const error = createError(
      'OptionError',
      translate('Contract type is not selected'),
    );
    globalObserver.emit('Error', error);
    throw error;
  }
...
export const expectCandle = candle => {
  if (!isCandle(candle)) {
    const error = createError(
      'CandleExpected',
      translate('Given candle is not valid'),
    );
    globalObserver.emit('Error', error);
    throw error;
  }
...
export const expectCandles = candles => {
  if (!(candles instanceof Array) || !candles.every(c => isCandle(c))) {
    const error = createError(
      'CandleListExpected',
      translate('Given candle list is not valid'),
    );
    globalObserver.emit('Error', error);
    throw error;
  }
