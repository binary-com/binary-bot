...
export default class TicksService {
    ...
    return new Promise(resolve => {
      this.api.getActiveSymbolsBrief().then(r => {
        ...
        this.pipSizes = symbols
          .reduce(
            (s, i) => s.set(i.symbol, +(+i.pip).toExponential().substring(3)),
            new Map(),
          )
          .toObject();
  ...
  stopMonitor(options) {
    ...
    if (
      type === 'candles' && this.ohlcListeners.hasIn([symbol, granularity, key])
    ) {
  ...
  unsubscribeAllAndSubscribeListeners(symbol) {
    ...
    Promise.all(
      subscription.map(id => doUntilDone(() => this.api.unsubscribeByID(id))),
    ).then(() => {
      ...
      if (this.tickListeners.has(symbol)) {
        this.ticks = this.ticks.delete(symbol);
        this.requestStream({ symbol, style: 'ticks' }).catch(e =>
    ...
    this.api.events.on('ohlc', r => {
      ...
      if (this.candles.hasIn([symbol, granularity])) {
        ...
        this.candles = this.candles.setIn(
          [symbol, granularity],
          updateCandles(
            this.candles.getIn([symbol, granularity]),
            parseOhlc(ohlc),
          ),
        );
