...
Blockly.Blocks.procedures_defnoreturn = {
    init() {
        ...
        const fieldImage = new Blockly.FieldImage(plusIconLight, 24, 24, '+', () => this.onAddClick_());
            ...
            Blockly.prompt(promptMessage, '', paramName => {
                if (paramName) {
                    ...
                    if (variable) {
                        ...
                        paramField.setText(`${translate('with: ')} ${this.arguments_.join(', ')}`);
                    }
                }
            });
        }, 200);
    ...
    updateParams_() {
        ...
        if (this.arguments_.length) {
            paramString = `${translate('with:')} ${this.arguments_.join(', ')}`;
        }
    ...
    mutationToDom(opt_paramIds) {
        ...
        this.argumentVarModels_.forEach((arg, i) => {
    ...
    getProcedureCallers() {
        return this.workspace
            .getAllBlocks(false)
            .filter(block => block.type === this.callType_ && block.data === this.id);
    },
    /**
    ...
    /**
     * TODO: Registers an event to the workspace this procedure is on once.
     * This listener will clean up variables that aren't used by any procedure
     * and that aren't created by the user. This listener will persist through
     * block deletion.
     * @this Blockly.Block
     */
    registerWorkspaceListener() {
        ...
        const hasProcedureListener = this.workspace.listeners_.find(func => func.name === 'workspaceListener');
        if (!hasProcedureListener) {
            this.workspace.addChangeListener(workspaceListener);
        }
    },
};
...
Blockly.JavaScript.procedures_defnoreturn = block => {
    ...
    const args = block.arguments_.map(argumentName =>
        Blockly.JavaScript.variableDB_.getName(argumentName, Blockly.Variables.NAME_TYPE)
    );
