...
Blockly.Blocks.read_ohlc = {
  init: function init() {
    ...
    this.setTooltip(translator.translateText('Read a field from a candle selected by index from the last candle, 1 as the latest and 2 as the candle before that.'));
...
Blockly.JavaScript.read_ohlc = (block) => {
  ...
  let index = Number(Blockly.JavaScript.valueToCode(block, 'CANDLEINDEX', Blockly.JavaScript.ORDER_ATOMIC));
  let code;
  if (!index) {
    index = 1;
  }
