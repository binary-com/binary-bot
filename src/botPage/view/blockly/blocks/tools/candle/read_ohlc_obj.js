...
Blockly.Blocks.read_ohlc_obj = {
  init: function init() {
    this.appendValueInput('OHLCOBJ')
      ...
      .appendField(
        new Blockly.FieldDropdown(config.ohlcFields),
        'OHLCFIELD_LIST',
      )
    ...
    this.setTooltip(
      translate('Read a field in a candle (read from the Candles list)'),
    );
...
Blockly.JavaScript.read_ohlc_obj = block => {
  ...
  const ohlcObj = Blockly.JavaScript.valueToCode(
    block,
    'OHLCOBJ',
    Blockly.JavaScript.ORDER_ATOMIC,
  );
  return [
    `Bot.candleField(${ohlcObj}, '${ohlcField}')`,
    Blockly.JavaScript.ORDER_ATOMIC,
  ];
};
