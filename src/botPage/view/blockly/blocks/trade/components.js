...
export const marketDropdown = block => {
  ...
  const getSymbols = () => {
    ...
    return Object.keys(symbols).map(e => [
      symbols[e].display,
      symbols[e].symbol,
    ]);
  };
...
export const tradeTypeDropdown = block => {
  const getTradeTypeCats = () => {
    ...
    const allowedCategories = symbolApi.getAllowedCategories(
      symbol.toLowerCase(),
    );
  ...
  const getTradeTypes = () => {
    ...
    return config.conditionsCategory[tradeTypeCat].map(e => [
      config.opposites[e.toUpperCase()]
        .map(c => c[Object.keys(c)[0]])
        .join('/'),
      e,
    ]);
  };
  block
    ...
    .appendField(
      new Blockly.FieldDropdown(getTradeTypeCats),
      'TRADETYPECAT_LIST',
    )
    .appendField('->')
...
export const candleInterval = block => {
  if (!block.getInput('CANDLE_INTERVAL')) {
    block
      ...
      .appendField(
        new Blockly.FieldDropdown(config.candleIntervals),
        'CANDLEINTERVAL_LIST',
      );
  }
};
...
export const duration = block => {
  if (!block.getInput('DURATION')) {
    ...
    block
      ...
      .appendField(
        new Blockly.FieldDropdown(getDurationTypes),
        'DURATIONTYPE_LIST',
      );
  }
};
...
export const payout = block => {
  if (!block.getInput('AMOUNT')) {
    block
      ...
      .appendField(
        new Blockly.FieldDropdown(config.lists.CURRENCY),
        'CURRENCY_LIST',
      );
  }
};
