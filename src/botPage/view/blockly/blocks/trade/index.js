...
Blockly.Blocks.trade = {
  init: function init() {
    ...
    this.setTooltip(
      translate(
        'Define your trade contract and start the trade, add initializations here. (Runs on start)',
      ),
    );
    ...
    if (ev.blockId === this.id) {
      ...
      if ([Blockly.Events.CREATE, Blockly.Events.CHANGE].includes(ev.type)) {
        ...
        if (!this.isInFlyout) {
          ...
          const submarket = this.getInput('SUBMARKET').connection
            .targetConnection;
          const needsMutation =
            submarket && submarket.sourceBlock_.type !== 'tradeOptions';
...
Blockly.JavaScript.trade = block => {
  ...
  const initialization = Blockly.JavaScript.statementToCode(
    block,
    'INITIALIZATION',
  );
  const tradeOptionsStatement = Blockly.JavaScript.statementToCode(
    block,
    'SUBMARKET',
  );
