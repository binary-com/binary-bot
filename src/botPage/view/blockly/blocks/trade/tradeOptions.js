...
export default () => {
    ...
    onchange: function onchange(ev) {
      ...
      if (ev.name === 'TRADETYPE_LIST') {
        if (ev.newValue) {
          updateInputList(this);
          this.setFieldValue(
            config.durationTypes[ev.newValue.toUpperCase()][0][1],
            'DURATIONTYPE_LIST',
          );
        } else {
  ...
  Blockly.JavaScript.tradeOptions = block => {
    ...
    const oppositesName = tradeDefBlock
    ...
    if (config.hasPrediction.indexOf(oppositesName) > -1) {
      predictionValue = Blockly.JavaScript.valueToCode(
        block,
        'PREDICTION',
        Blockly.JavaScript.ORDER_ATOMIC,
      );
    ...
    if (config.hasSecondBarrierOffset.indexOf(oppositesName) > -1) {
      const barrierOffsetType = block.getFieldValue(
