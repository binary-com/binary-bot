...
export default class _Blockly {
    ...
    this.initPromise = new Promise(resolve => {
      $.get('xml/toolbox.xml', toolboxXml => {
        ...
        const workspace = Blockly.inject('blocklyDiv', {
          toolbox: this.xmlToStr(
            translateXml(toolboxXml.getElementsByTagName('xml')[0]),
          ),
          zoom: {
            wheel: false,
          },
        ...
        $.get('xml/main.xml', main => {
          ...
          Blockly.Xml.domToWorkspace(
            main.getElementsByTagName('xml')[0],
            workspace,
          );
    ...
    topBlocks.forEach(block => {
      if (
        ...
        disable(
          block,
          translate(
            'Blocks must be inside block holders, main blocks or functions',
          ),
        );
      }
    });
  ...
  disposeBlocksWithLoaders() {
    Blockly.mainWorkspace.addChangeListener(ev => {
      ...
      if (
        ev.type === 'delete' &&
        ev.oldXml.getAttribute('type') === 'loader' &&
        ev.group !== 'undo'
      ) {
  ...
  resetWorkspace() {
    ...
    Blockly.mainWorkspace.clear();
    Blockly.Xml.domToWorkspace(
      Blockly.Xml.textToDom(this.blocksXmlStr),
  ...
  load(blockStr = '', dropEvent = {}) {
    ...
    try {
      ...
      throw createError(
        'FileLoad',
        translate('Unable to load the block file.'),
      );
    }
  }
  save(arg) {
    ...
    Array.from(xml.children).forEach(blockDom => {
      const block = Blockly.mainWorkspace.getBlockById(
        blockDom.getAttribute('id'),
      );
