...
const isInRange = (amount, min, max) =>
  ...
  if (
    ...
    if (duration) {
      ...
      if (!durationInSeconds) {
        globalObserver.emit(
          'ui.log.warn',
          translate('Duration must be a positive integer'),
        );
        ...
        durationType === 't' &&
        !(Number.isInteger(duration) && isInRange(duration, 5, 10))
      ) {
        globalObserver.emit(
          'ui.log.warn',
    ...
    if (prediction) {
      ...
      if (!Number.isInteger(prediction) || !isInRange(prediction, 0, 9)) {
        globalObserver.emit(
          'ui.log.warn',
          translate('Prediction must be one digit'),
        );
      }
    }
  }
