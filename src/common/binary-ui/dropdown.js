...
function topNavMenuListener() {
  $('.top-nav-menu > li').on('click', function(event) {
    ...
    if (
      ...
      $(event.target).find('span').hasClass('nav-caret')
    ) {
      hide_menu($el);
    } else if (
...
function initMenuContent(_menu_containers) {
  ...
  _menu_containers
    .filter(':not(.follow-default)')
    .delegate('.tm-a,.tm-a-2', 'click', function(event) {
      ...
      if (tab_id) {
        ...
        var selected_tab =
          ...
          tab_container
            ...
            .find('.tm-a')
            .wrap(
              '<span class="menu-wrap-a"><span class="menu-wrap-b"></span></span>',
            )
            .end()
        ...
        span_tm_a.replaceWith(
          '<a href="#" class="' +
            span_tm_a.attr('class') +
            '">' +
            span_tm_a.html() +
            '</a>',
        );
