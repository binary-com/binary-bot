...
RelationChecker.prototype.getNumField = function getNumField(block, fieldName) {
...
RelationChecker.prototype.isInteger = function isInteger(amount){
	return !isNaN(+amount) && parseInt(amount) === parseFloat(amount); 
};

RelationChecker.prototype.isInRange = function isInRange(amount, min, max) {
	return !isNaN(+amount) && +amount >= min && +amount <= max;
};

RelationChecker.prototype.trade = function trade(_trade, ev){
	if ( ev.type === 'create' ) {
...
RelationChecker.prototype.submarket = function submarket(_submarket, ev){
...
RelationChecker.prototype.condition = function condition(_condition, ev, calledByParent){
		...
		if ( Bot.config.ticktrade_markets.indexOf(_condition.parentBlock_.type) < 0 ) {
			...
			if ( !calledByParent) {
				if ( ( ev.type === 'change' && ev.element && ev.element === 'field' ) || ( ev.type === 'move' && typeof ev.newInputName === 'string' ) ){
					var added = []; 
					var duration = this.getNumField(_condition, 'DURATION');
					if ( duration !== '' ) {
						if ( !this.isInteger(duration) || !this.isInRange(duration, 5, 15) ) {
					...
					var amount = this.getNumField(_condition, 'AMOUNT');
					if ( amount !== '' ) {
						if ( !this.isInRange(amount, 0.35, maximum) ) {
							Bot.utils.log('Ticks has to be an integer between 0.35 and ' + maximum, 'warning');
						} else {
							added.push('AMOUNT');
						}
					}
					var prediction = this.getNumField(_condition, 'PREDICTION');
					if ( prediction !== '' ) {
...
RelationChecker.prototype.purchase = function purchase(_purchase, ev) {
...
RelationChecker.prototype.trage_again = function trade_again(_trade_again, ev) {
